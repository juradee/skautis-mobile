...
## Tenhle projekt: skautIS mobil

Mobilní PWA nad webovými službami skautISu. Čti `README.md` – popisuje běh v Dockeru,
demo režim a strukturu.

Repozitář je **dvě věci pod jednou licencí každá**:

- kořen = aplikace, **AGPL-3.0**;
- `packages/skautis-symfony/` = samostatný Symfony bundle `webwingscz/skautis-symfony`,
  **MIT**, připravený k vydání na Packagist. Aplikace ho tahá přes Composer path
  repository. Kód, který má smysl i mimo tuhle aplikaci, patří do balíčku; controllery,
  šablony a PWA zůstávají v aplikaci.

Čtyři věci, které se tu dělají jinak, než by člověk čekal:

- **Na hostiteli není PHP ani Composer.** Všechno jde přes `make` (tj. `docker compose`).
  Docker běží v rootless režimu, takže kontejnery pracují jako `root` – to je záměr,
  jinak by nešlo psát do bind-mountu.
- **skautIS je SOAP, ne REST.** Nepřidávej `skautis/skautis` z Packagistu; klient je
  v `packages/skautis-symfony/src/Client/`. Názvy vstupních polí ověřuj vždy proti
  `https://test-is.skaut.cz/JunakWebservice/<Služba>.asmx?op=<Metoda>`, ne od oka.
  `LoginUpdate` a `LoginUpdateRefresh` berou token jako `ID`, zbytek jako `ID_Login`.
- **Bez APP_ID se jede na fixtures.** `SKAUTIS_MOCK=1` přepne `SkautisClientInterface`
  na `FixtureSkautisClient`, který čte `packages/skautis-symfony/resources/fixtures/`.
